...
public class ReservationServiceUnitTest {
    ...
    public void postPaymentTest() {
        // given
        Long userId = 1L;
        PaymentHistory paymentHistory = new PaymentHistory(1L, 1L, userId, 180000L, PaymentStatus.SUCCESS, LocalDateTime.of(2024, 6, 28, 12, 0));

        // when
        when(reservationService.postPayment(userId, paymentHistory)).thenReturn(paymentHistory);

        // then
        assertThat(reservationService.postPayment(userId, paymentHistory).getPaymentId()).isEqualTo(1L);
        assertThat(reservationService.postPayment(userId, paymentHistory).getPaymentTime()).isEqualTo("2024-06-28T12:00:00");
    }
}
